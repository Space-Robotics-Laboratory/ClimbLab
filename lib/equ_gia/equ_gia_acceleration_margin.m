...
%
%     OUTPUT
%         gia_margin       : Acceleration margin considering the stability polyhedron [m/s^2] (scalar)
%         gia_margin_ab    : Acceleration margin for each tumbling axis [m/s^2] (1xn vector)
%     INPUT
...
function [gia_margin, gia_margin_ab] = equ_gia_acceleration_margin(polyhedron, gia, equ_flag)

% Number of tumbling axes
tumbling_axes_number = size(polyhedron.plane_vector,2);
% Initialize variable
gia_margin_ab = zeros(1,tumbling_axes_number);
...
if equ_flag == 0
    ...
    for i = 1:tumbling_axes_number
        ...
        gia_margin_ab(i) = norm(polyhedron.plane_point(:,i)) - gia'*polyhedron.plane_vector(:,i)/norm(polyhedron.plane_vector(:,i));
    end
    gia_margin = min(gia_margin_ab);
end
