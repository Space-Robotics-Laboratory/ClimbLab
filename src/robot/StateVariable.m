classdef StateVariable
  ...
  methods (Access = ?Robot)
    ...
    function SV = StateVariable(num_joints)
    % StateVariable() Constructor
    %   Input - num_joints: Total number of joints
      arguments (Input)
        num_joints (1, 1) {mustBeA(num_joints, "double")};
      end
      SV.q   = zeros(num_joints, 1);
      SV.qd  = zeros(num_joints, 1);
      SV.qdd = zeros(num_joints, 1);
      ...
      SV.vv = zeros(3, num_joints);
      SV.ww = zeros(3, num_joints);
      SV.vd = zeros(3, num_joints);
      SV.wd = zeros(3, num_joints);
      ...
      SV.Fe = zeros(3, num_joints);
      SV.Te = zeros(3, num_joints);
      SV.F0 = zeros(3, 1);
      SV.T0 = zeros(3, 1);

      SV.tau = zeros(num_joints, 1);

      SV.RR = zeros(3, num_joints);
      SV.AA = zeros(3, 3 * num_joints);
    end
  ...
  methods (Access = public)
    ...
    function SV = setJointAnglularPositions(SV, joint_angles)
      arguments (Input)
        SV;
