  ...
  methods (Access = public)
    ...
    % GraphPlotter() Constructor
      ...
      for i = 1 : length(kConfigPropName)
        addprop(graph_plotter, kConfigPropName{i, 1});
    ...
    function plot(graph_plotter, robot, variables_log)
    % plot()
    %   Plot graphs
      arguments (Input)
        graph_plotter;
        robot (1, 1) {mustBeA(robot, "Robot")};
        variables_log (1, 1) {mustBeA(variables_log, "struct")};
      end
      ...
      if (graph_plotter.kPlotBasePosition_)
        ...
        graph_plotter.plotTimeHistoryGraph(variables_log.time, variables_log.base_position, fig_title, y_label);
      end
      ...
      if (graph_plotter.kPlotTumbleStabilityMargin_)
        ...
        graph_plotter.plotTimeHistoryGraph(variables_log.time, variables_log.TSM, fig_title, y_label);
      end
    end
