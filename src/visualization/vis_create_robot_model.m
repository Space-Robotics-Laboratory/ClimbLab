...
%         shape_robot.base_upper_V : Vertices for upper part of the base (8x3)
%         shape_robot.base_upper_F : Faces numbering for upper part of the base (6x4 matrix)
%         shape_robot.base_lower_V : Vertices for lower part of the base (8x3)
%         shape_robot.base_lower_F : Faces numbering for lower part of the base (6x4 matrix)
%     INPUT 
%         LP                 : Link parameters (SpaceDyn class)
%         LP.F_grip          : Maximum gripping force [N] (scalar)
%         ani_settings.link_radius    : Radius for the links [m] (scalar)
%         ani_settings.base_upper_thickness : Upper thickness of the base [m] (scalar)
%         ani_settings.base_lower_thickness : Lower thickness of the base [m] (scalar)

function shape_robot = vis_create_robot_model(ani_settings, LP)
...
%   end
  ...
  % Create base
  % upper box of the base
  scale = ani_settings.base_xy_scale_factor;
  for i = 1:LP.num_limb
      shape_robot.base_upper_V(i,:) = [LP.c0(1:2,3*i-2)'*scale   ani_settings.base_upper_thickness];
  end
  for i = 1:LP.num_limb
      shape_robot.base_upper_V(LP.num_limb+i,:) = [LP.c0(1:2,3*i-2)'*scale 0];
  end
  shape_robot.base_upper_F = [1 2 3 4; 1 2 6 5; 2 3 7 6; 3 4 8 7; 1 4 8 5; 5 6 7 8];
  
  % lower box of the base
  for i = 1:LP.num_limb
      shape_robot.base_lower_V(i,:) = [LP.c0(1:2,3*i-2)'*scale 0];
  end
  for i = 1:LP.num_limb
      shape_robot.base_lower_V(LP.num_limb+i,:) = [LP.c0(1:2,3*i-2)'*scale -ani_settings.base_lower_thickness];
  end
  shape_robot.base_lower_F = [1 2 3 4; 1 2 6 5; 2 3 7 6; 3 4 8 7; 1 4 8 5; 5 6 7 8];
