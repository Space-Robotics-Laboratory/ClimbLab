...
%%%%%% vis_cylinder
%%%%%%
%%%%%% Visualize the cylinder connect any two points X1 and X2
%%%%%%
%%%%%% Created:      2021.01.19 by Warley Ribeiro
%%%%%% Last updated: 2021.01.19 by Kentaro Uno
%
% Note: this function is created based on the following function. Based on
% the functon, the transparency of the face is allowed to be editable.
% ------------------------------------------------------------------------
% J. Divahar (2021). Cylinder (https://www.mathworks.com/matlabcentral/fileexchange/13995-cylinder),
% MATLAB Central File Exchange. Retrieved January 19, 2021.
% ------------------------------------------------------------------------
%
function [Cylinder EndPlate1 EndPlate2] = vis_cylinder(X1,X2,r,n,cyl_color,cyl_alpha,closed,lines)
%
...
if closed==1
    hold on
    EndPlate1=fill3(xx1(:,1),xx2(:,1),xx3(:,1),'r', "Visible", "off");
    EndPlate2=fill3(xx1(:,2),xx2(:,2),xx3(:,2),'r', "Visible", "off");
end
% Plotting the cylinder along the X-Direction with required length starting
% from Origin
Cylinder=mesh(xx1,xx2,xx3, "Visible", "off");
...
set(Cylinder,'FaceColor',cyl_color,'Edgecolor','k','FaceAlpha',cyl_alpha);
if closed==1
    set([EndPlate1 EndPlate2],'FaceColor',cyl_color,'Edgecolor','none','FaceAlpha',cyl_alpha);
else
    EndPlate1=[];
    EndPlate2=[];
end
