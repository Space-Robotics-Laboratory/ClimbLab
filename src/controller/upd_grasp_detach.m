...
function SV = upd_grasp_detach(LP, SV, des_SV, environment_param, equilibrium_eval_param, variables_saved)
...
switch environment_param.detachment_detection_method
    ...
    case 'max_holding_force'
        Force = SV.Fe(:,LP.SE == 1);
        for i = 1:LP.num_limb
            if norm(Force(:,i)) > LP.F_grip  &&  SV.Fe(3,3*i) < 0  &&  des_SV.sup(i) == 1
                SV.slip(i) = 1;
            end
        end
