%%%%%% Update
%%%%%% upd_swing_current_des_POS_spline3_two_segments_time
%%%%%% 
...
function motion_planning_param = upd_swing_current_des_POS_spline3_two_segments_time(gait_planning_param, motion_planning_param, LP, cont_POS, time)

for i = 1:LP.num_limb
    % Current swing limb
    if i == gait_planning_param.swing_number
        % Check if before or after mid-point
        if time <= 0.5*(gait_planning_param.leg_T(1,i) + gait_planning_param.leg_T(2,i))
            j = 1;
        else
            j = 2; 
        end
