%%%%%% Calculate
%%%%%% get_i_kine_insect_config_3dof_limb
%%%%%% 
%%%%%% - Position inverse kinematics solver for a 3 DOF manipulator that
%%%%%% has a insect joint configuration
%%%%%% 
%%%%%% Created: 2020-06-25
%%%%%% Kentaro Uno
...
function q_sol = get_i_kine_insect_config_3dof_limb( LP, SV, POS_e, num_e )
...
%Singurarity check
if isreal(sqrt(a1^2+b1^2-c1^2)) == false %&& imag(sqrt(a1^2+b1^2-c1^2)) > 3*10e-3
    return
end
...
%Singurarity check
if isreal(sqrt(a2^2+b2^2-c2^2)) == false %&& imag(sqrt(a2^2+b2^2-c2^2)) > 3*10e-3
    return
end
